...
export class SocialMediaStudiosService {
  ...
  async create(payload: CreateSocialMediaStudioDto, currentUser): Promise<ResponseData> {
    ...
    const {users, ...socialMediaStudioParams} = socialMediaStudioPayload
    ...
    try {
      let usersUnify = _.uniqBy(users,'id')
      let userIds = usersUnify.map(e=>e.id)
      let myUser = usersUnify.find(e => {return e.id === currentUser.id})
      if (!userIds.includes(currentUser.id) && myUser) {
        usersUnify.push({id: currentUser.id,canEdit: true, approved: false})
      } else {
        myUser.canEdit = true
        myUser.approved = myUser.approved === undefined ? true : myUser.approved
      }
      console.log(myUser, usersUnify)
      ...
      if (createdSocialMediaStudio) {
        const userfnd = await this.userModel.query(trx).select('id').whereIn('id', userIds).whereIn('users.userType',['owner','admin','agent']).where({brandCode: currentUser.brandCode})
        if (!userfnd.length || userfnd.length !== userIds.length ) {
          throw 'User Error: A user doesnt exist.'
        }
        for (let item of usersUnify) {
          let smsUser: SocialMediaStudioUserDto = item
          const usersParams: CreateSocialMediaStudioUserDto = {
            userId: smsUser.id,
            socialMediaStudioId: createdSocialMediaStudio.id,
            canEdit: smsUser.canEdit,
            approved: false,
  ...
  async approve(id: number, currentUser): Promise<ResponseData> {
    ...
    .where({ approved: false, canEdit: true, userId: currentUser.id })
    return {
      success: true,
  ...
  async update(payload: UpdateSocialMediaStudioDto, currentUser): Promise<ResponseData> {
    ...
    if (socialMediaStudio) {
      ...
      if (socialMediaStudio.creatorId === currentUser.id) {
        ...
        try {
          // start operation for updating SocialMediaStudio and leaders and users
          const oldSocialMediaStudioUsers = this.socialMediaStudioUserModel.query(trx)
          .select('id')
          .whereNotIn('id',_.uniq(users))
          .where({socialMediaStudioId: socialMediaStudioPayload.id});

          await oldSocialMediaStudioUsers.delete();
          const userfnd = await this.userModel.query(trx)
          .select('id')
          .whereIn('id', _.uniq(users))
          .where({brandCode: currentUser.brandCode});

          if (!userfnd.length && userfnd.length === _.uniq(users).length ) {
            return {
              success: false,
              message: 'User Error: A user doesnt exist.',
              data: {},
            };
          }

          await _.uniq(users).forEach(async (user: number) => {
            const usersParams: {userId: number,socialMediaStudioId: number, createdBy: string} ={
              userId: user,
              socialMediaStudioId: socialMediaStudioPayload.id,
              createdBy: currentUser.username
            }
            let finishedInsert = await this.socialMediaStudioUserModel.query(trx).insert(usersParams)
            if (!finishedInsert) {
      ...
      // } else if (socialMediaStudio.creatorId !== currentUser.id && socialMediaStudio.clientId && socialMediaStudio.client && socialMediaStudio.client.userId === currentUser.id && socialMediaStudio.stage === 'review') {
      //   if (socialMediaStudioPayload.stage === 'completed') {
      //     finalParams['actualdEndDate'] = moment().format('YYYY-MM-DD HH:mm:ss').toString()
      //     finalParams['stage'] = 'completed'
      //     finalParams['status'] = 'done'
      //   } else if (socialMediaStudioPayload.stage === 'rejected') {
      //     finalParams['status'] = 'rejected'
      //   }
      } else {
        return {
