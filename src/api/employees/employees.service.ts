...
export class EmployeesService {
  ...
  async findUsers(currentUser): Promise<ResponseData> {
  ...
  //   if ((getUserType(currentUser) !== UserLayers.layerOne) && (!currentUser.myEmployeeProfile || currentUser.myEmployeeProfile.hrMember != true)) {
  //     console.log("hr create error",getUserType(currentUser), currentUser.myEmployeeProfile)
  ...
  async create(payload:CreateEmployeeDto, currentUser): Promise<ResponseData> {
    if ((getUserType(currentUser) !== UserLayers.layerOne) && (!currentUser.myEmployeeProfile || currentUser.myEmployeeProfile.hrMember != true)) {
      console.log("normal employee create error",getUserType(currentUser), currentUser.myEmployeeProfile)
      throw new UnauthorizedException()
    }
    const newEmployee = await this.modelClass.query()
    .where({ brandCode: currentUser.brandCode })
